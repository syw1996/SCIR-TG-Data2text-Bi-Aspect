...
def make_features(batch, side, data_type='text'):
    ...
    Returns:
        ...
        of size (len x batch).
    """
    assert side in ['src1','src2', 'tgt1', 'tgt2', 'src1_char', 'src2_char', 'ref_src', 'ref_tgt', 'ref_src_char']
    ...
    if data_type == 'text' and not char_flag:
...
def build_dataset(fields, data_type, src_path, tgt_path, src_path2, tgt_path2, ref_src_path, ref_tgt_path, src_dir=None,
                  src_seq_length=0, tgt_seq_length=0,
                  src_seq_length_trunc=0, tgt_seq_length_trunc=0,
                  dynamic_dict=True, sample_rate=0,
                  window_size=0, window_stride=0, window=None,
                  normalize_audio=True, use_filter_pred=True, pointers_file=None, ref_pointers_file=None):
    ...
    ref_src_examples_iter, num_ref_src_feats = \
        _make_examples_nfeats_tpl(data_type, ref_src_path, src_dir,
                                  src_seq_length_trunc, sample_rate,
                                  window_size, window_stride,
                                  window, normalize_audio, "ref_src")

    ref_tgt_examples_iter, num_ref_tgt_feats = \
        TextDataset.make_text_examples_nfeats_tpl(
            ref_tgt_path, tgt_seq_length_trunc, "ref_tgt")

    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter, src_examples_iter2, tgt_examples_iter2,
                              ref_src_examples_iter, ref_tgt_examples_iter,
                              ...
                              ref_pointers_file=ref_pointers_file)
    elif data_type == 'img':
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    fields["ref_tgt"] = fields["tgt2"]

    if data_type == 'text':
        for src in ("src1", "src2"):
            if src not in fields.keys():
                continue
            ...
            _build_field_vocab(fields[src + "_char"], counter[src + "_char"],
                               max_size=src_vocab_size,
                               min_freq=src_words_min_frequency)
            print(" * %s vocab size: %d." % (src + "_char", len(fields[src + "_char"].vocab)))
        ...
        fields["ref_src"] = fields["src1"]
        fields["ref_src_char"] = fields["src1_char"]
        iter_range = dataset.n_src_feats
        for j in range(iter_range):
            key = "src1" + "_feat_" + str(j)
            ref_key = "ref_src" + "_feat_" + str(j)
            fields[ref_key] = fields[key]
