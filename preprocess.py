...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, src_corpus2, tgt_corpus2, ref_src_corpus, ref_tgt_corpus, fields,
                                      corpus_type, opt, pointers, ref_pointers):
    '''
    ...
    ref_src_iter = onmt.io.ShardedTextCorpusIterator(
                ref_src_corpus, opt.src_seq_length_trunc,
                "ref_src", opt.max_shard_size)
    ref_tgt_iter = onmt.io.ShardedTextCorpusIterator(
                ref_tgt_corpus, opt.tgt_seq_length_trunc,
                "ref_tgt", opt.max_shard_size,
                assoc_iter=ref_src_iter)

    index = 0
    while not src_iter.hit_end():
        index += 1
        dataset = onmt.io.TextDataset(
                fields, src_iter, tgt_iter, src_iter2, tgt_iter2, ref_src_iter, ref_tgt_iter,
                src_iter.num_feats, tgt_iter.num_feats, src_iter2.num_feats, tgt_iter2.num_feats, 
                ref_src_iter.num_feats, ref_tgt_iter.num_feats,
                src_seq_length=opt.src_seq_length,
                tgt_seq_length=opt.tgt_seq_length,
                dynamic_dict=opt.dynamic_dict, pointers_file=pointers, ref_pointers_file=ref_pointers)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if corpus_type == 'train':
        ...
        ref_src_corpus = opt.train_ref_src
        ref_tgt_corpus = opt.train_ref_tgt
        pointers = opt.train_ptr
        ref_pointers = opt.train_ref_ptr
    else:
        src_corpus = opt.valid_src1
        tgt_corpus = opt.valid_tgt1
        src_corpus2 = opt.valid_src2
        tgt_corpus2 = opt.valid_tgt2
        ref_src_corpus = opt.valid_ref_src
        ref_tgt_corpus = opt.valid_ref_tgt
        pointers = None
        ref_pointers = None
    ...
    if opt.data_type == 'text':
        return build_save_text_dataset_in_shards(
                src_corpus, tgt_corpus, src_corpus2, tgt_corpus2, ref_src_corpus, ref_tgt_corpus, fields,
                corpus_type, opt, pointers=pointers, ref_pointers=ref_pointers)
    ...
    dataset = onmt.io.build_dataset(
                fields, opt.data_type, src_corpus, tgt_corpus, src_corpus2, tgt_corpus2, ref_src_corpus, ref_tgt_corpus,
                src_dir=opt.src_dir,
                src_seq_length=opt.src_seq_length,
                tgt_seq_length=opt.tgt_seq_length,
